...
Foo and bar are references to the same object `{n: 1}`, lvalues are resolved before assignations. `foo = {n: 2}` is creating a new object, and foo is updated to reference that new object. The trick here is foo in `foo.x = ...` as a lvalue was resolved beforehand and still reference the old `foo = {n: 1}` object, adding the x property. After that chain assignments, bar still reference the old foo object, but foo reference the new `{n: 2}` object, where x is not existing.
